...
There is a straightforward, principled way of doing this when the subspaces are of equal dimension, but that's not always (or even often) the case. For example, the dimension of $\text{Sp}(\bf{\hat{Y}})$ is bounded by the number of predictors (where we consider the intercept to be its own predictor), and so, when the number of predictors is smaller than the number of response variables, we will necessarily find ourselves in this position unless the response variables are linearly dependent. 

Ignore that issue for now, and we'll just consider the case where $\text{Sp}(\bf{Y})$ and $\text{Sp}(\bf{\hat{Y}})$ are equal. For example, suppose that we have two response variables (so that $\text{Sp}(\bf{Y})$ is a plane), and enough predictors
